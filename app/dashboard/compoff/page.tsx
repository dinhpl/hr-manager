...
export default function CompoffPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Khong the tai du lieu comp-off."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const quickStats = useMemo(() => {
    const sourceOtHours = items.reduce(
      (sum, item) => sum + Number(item.overtime?.hours ?? 0),
      0
    );
    const approvedCount = items.filter(
      (item) => getStatusLabel(item.status) === "approved"
    ).length;
    const pendingCount = items.filter(
      (item) => getStatusLabel(item.status) === "pending"
    ).length;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={() => setActionMessage("Khối tính toán ở cột phải chỉ dùng để ước tính nhanh.")}
      ...
      <div className="rounded-xl border bg-white p-5" style={{ borderColor: "#e2ede9" }}>
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <div className="flex h-5 overflow-hidden rounded-full" style={{ background: "#f0f4f2" }}>
              <div
          ...
          {[
            ...
            <div key={filter.id} className="flex flex-col gap-1">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  filter.onChange(value.startsWith("all-") ? "" : value)
                }
              >
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-3">
                    ...
                    <div className="h-2 overflow-hidden rounded-full" style={{ background: "#e2ede9" }}>
                      <div
