...
const APPROVER_ROLE_OPTIONS: ApprovalLevel["approverRole"][] = ["MANAGER", "HR", "ADMIN", "EMPLOYEE"];
...
function normalizeLeavePolicy(raw: Partial<LeavePolicy> | null | undefined): LeavePolicy {
  const rulesSource = Array.isArray(raw?.annualLeaveRules) && raw.annualLeaveRules.length
    ? raw.annualLeaveRules
    : DEFAULT_LEAVE_POLICY.annualLeaveRules;

  return {
    annualLeaveRules: rulesSource.map((rule, index) => ({
      fromYear: toSafeNumber(rule?.fromYear, DEFAULT_LEAVE_POLICY.annualLeaveRules[index]?.fromYear ?? 0),
    ...
    advanceRequestDays: toSafeNumber(raw?.advanceRequestDays, DEFAULT_LEAVE_POLICY.advanceRequestDays),
    maxConsecutiveDays: toSafeNumber(raw?.maxConsecutiveDays, DEFAULT_LEAVE_POLICY.maxConsecutiveDays),
  };
}

function normalizeApprovalFlow(raw: Partial<ApprovalFlow> | null | undefined): ApprovalFlow {
  const levelsSource = Array.isArray(raw?.levels) && raw.levels.length ? raw.levels : DEFAULT_APPROVAL_FLOW.levels;

  return {
    levels: levelsSource.map((level, index) => ({
      level: index + 1,
      approverRole: APPROVER_ROLE_OPTIONS.includes(level?.approverRole as ApprovalLevel["approverRole"])
        ? (level?.approverRole as ApprovalLevel["approverRole"])
        : DEFAULT_APPROVAL_FLOW.levels[index]?.approverRole ?? "MANAGER",
      timeLimit: toSafeNumber(level?.timeLimit, DEFAULT_APPROVAL_FLOW.levels[index]?.timeLimit ?? 24),
    })),
...
export default function SettingsPage() {
  ...
  const [documentTypesInput, setDocumentTypesInput] = useState(DEFAULT_APPROVAL_FLOW.requireDocumentTypes.join(", "));
  ...
  const handleSaveLeaveRule = (index: number) => {
    setLeavePolicy((prev) => ({
      ...prev,
      annualLeaveRules: prev.annualLeaveRules.map((rule, ruleIndex) => (ruleIndex === index ? leaveDraft : rule)),
    }));
    setEditingLeaveIndex(null);
  };
  ...
  const handleSaveApprovalLevel = (index: number) => {
    setApprovalFlow((prev) => ({
      ...prev,
      levels: renumberLevels(prev.levels.map((level, levelIndex) => (levelIndex === index ? levelDraft : level))),
    }));
    setEditingLevelIndex(null);
  };
  ...
  const handleCancelApprovalLevel = (index: number) => {
    ...
    const isNewLevel = target?.timeLimit === 24 && target?.approverRole === "MANAGER" && target?.level === approvalFlow.levels.length;
    if (isNewLevel) {
      setApprovalFlow((prev) => ({
        ...prev,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: "#e2ede9" }}>
        ...
        <div className="p-6">
          {isLoading ? (
            <div className="rounded-xl border border-dashed p-8 text-center text-sm" style={{ borderColor: "#d6e5df", color: "#6b7f78" }}>
                ...
                <div className="space-y-6">
                  <div className="border-l-4 border-blue-500 pl-4">
                    ...
                    <div className="grid md:grid-cols-3 gap-4 mb-6">
                      <div>
                        <label className="block text-xs font-semibold mb-2" style={{ color: "#6b7f78" }}>
                          Phép năm mặc định
                        </label>
                            ...
                            onChange={(event) =>
                              setLeavePolicy((prev) => ({
                                ...
                                annualLeaveRules: prev.annualLeaveRules.map((rule, index) =>
                                  index === 0 ? { ...rule, days: Number(event.target.value) } : rule
                                ),
                              }))
                            }
                      ...
                      <div>
                        ...
                        <Select value={leavePolicy.annualLeaveRules.length > 1 ? "milestone" : "none"} disabled>
                          <SelectTrigger>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {leavePolicy.annualLeaveRules.map((rule, index) => (
                            <tr key={`${rule.fromYear}-${rule.toYear}-${index}`} className="border-b last:border-0" style={{ borderColor: "#f0f4f2" }}>
                              <td className="px-3 py-3">
                                {editingLeaveIndex === index ? (
                                  <input
                                    ...
                                    onChange={(event) =>
                                      setLeaveDraft((prev) => ({ ...prev, fromYear: Number(event.target.value) }))
                                    }
                              ...
                              <td className="px-3 py-3">
                                {editingLeaveIndex === index ? (
                                  <input
                                    ...
                                    onChange={(event) =>
                                      setLeaveDraft((prev) => ({ ...prev, toYear: Number(event.target.value) }))
                                    }
                                    className="w-full px-2 py-1 rounded border text-sm"
                                    style={{ borderColor: "#e2ede9" }}
                                  />
                              ...
                              <td className="px-3 py-3">
                                {editingLeaveIndex === index ? (
                                  <input
                                    ...
                                    onChange={(event) =>
                                      setLeaveDraft((prev) => ({ ...prev, days: Number(event.target.value) }))
                                    }
                                    className="w-full px-2 py-1 rounded border text-sm"
                                    style={{ borderColor: "#e2ede9" }}
                                  />
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setLeavePolicy((prev) => ({
                              ...prev,
                              carryOverLimit: checked === true ? Math.max(prev.carryOverLimit, 1) : 0,
                            }))
                          }
                ...
                <div className="space-y-6">
                  <div className="flex justify-center gap-3 py-6 px-4 rounded-lg overflow-x-auto" style={{ background: "#f0f4f2" }}>
                    {workflowSteps.map((step, index) => (
                      <div key={`${step.level}-${step.approverRole}`} className="flex items-center gap-3 shrink-0">
                        <div
                        ...
                        {index < workflowSteps.length - 1 && <div className="text-xl" style={{ color: "#6b7f78" }}>→</div>}
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {approvalFlow.levels.map((level, index) => (
                        <div key={`${level.level}-${level.approverRole}`}>
                          <label className="block text-sm font-medium mb-2" style={{ color: "#203430" }}>
                  ...
                  <div className="border-l-4 border-red-500 pl-4">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr style={{ background: "#f7f7f7" }}>
                            {["Cấp", "Vai trò duyệt", "Thời gian chờ", "Trạng thái", "Thao tác"].map((heading) => (
                              <th
                                key={heading}
                                className="px-3 py-2.5 text-left text-xs font-semibold"
                                style={{ color: "#6b7f78" }}
                        ...
                        <tbody>
                          {approvalFlow.levels.map((level, index) => (
                            <tr key={`${level.level}-${level.approverRole}-${index}`} className="border-b last:border-0" style={{ borderColor: "#f0f4f2" }}>
