...
export default function LeaveDetailModal({ data, onClose }: LeaveDetailModalProps) {
  ...
  const statusInfo = STATUS_CONFIG[data.status] ?? { label: data.status, bg: "#f3f4f6", color: "#6b7280" };
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-5">
          ...
          {data.employeeName && (
            <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "#f0f9f5" }}>
              <div
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm" style={{ color: "#203430" }}>
                  {data.employeeName}
                  {data.employeeCode && (
                    <span className="font-normal ml-1" style={{ color: "#6b7f78" }}>({data.employeeCode})</span>
                  )}
                </p>
                {data.employeeTeam && (
                  <p className="text-xs truncate" style={{ color: "#6b7f78" }}>{data.employeeTeam}</p>
                )}
              </div>
              {data.leaveBalance && (
                <div className="text-right flex-shrink-0">
                  <p className="text-xs" style={{ color: "#6b7f78" }}>Phép còn lại</p>
                  <p className="font-bold" style={{ color: "#1DB87A" }}>{data.leaveBalance} ngày</p>
                </div>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center p-3 rounded-xl" style={{ background: "#f7f7f7" }}>
                ...
                <p className="font-semibold text-sm" style={{ color: "#203430" }}>{data.fromDate}</p>
              </div>
              <div className="text-center p-3 rounded-xl" style={{ background: "#f7f7f7" }}>
                <p className="text-xs mb-1" style={{ color: "#6b7f78" }}>Đến ngày</p>
                <p className="font-semibold text-sm" style={{ color: "#203430" }}>{data.toDate}</p>
              </div>
              <div className="text-center p-3 rounded-xl" style={{ background: "#D3F2E7" }}>
                <p className="text-xs mb-1" style={{ color: "#0E474E" }}>Số ngày</p>
                <p className="font-bold text-lg" style={{ color: "#1DB87A" }}>{data.days}</p>
              </div>
            </div>
          </div>
          ...
          {data.reason && (
            <div>
              ...
              <p className="text-sm p-3 rounded-xl" style={{ background: "#f7f7f7", color: "#6b7f78" }}>
                {data.reason}
              </p>
            </div>
          )}
          ...
          {data.handover && data.handover !== "-" && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold" style={{ color: "#203430" }}>{data.handover}</span>
            </div>
          )}
          ...
          {data.fileAttachment && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold" style={{ color: "#3b82f6" }}>{data.fileAttachment}</span>
            </div>
          )}
          ...
          {data.submittedAt && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium" style={{ color: "#203430" }}>{data.submittedAt}</span>
            </div>
          )}
          ...
          {data.approver && data.approver !== "-" && (
            ...
            >
              ...
              <p className="text-sm font-semibold" style={{ color: "#203430" }}>{data.approver}</p>
              {data.approverRole && (
                <p className="text-xs" style={{ color: "#6b7f78" }}>{data.approverRole}</p>
              )}
